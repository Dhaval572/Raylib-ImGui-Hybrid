    ...
    Model ModelLoader::LoadFBX(std::string_view Path)
    {
        ...
        for (size_t i = 0; i < scene->nodes.count; i++)
        {
            ...
            if (node->mesh)
            {
                    ...
                    for (size_t v = 2; v < face.num_indices; v++)
                    {
                        ...
                        for (int k = 0; k < 3; k++)
                        {
                            ...
                            if (ix < fbx_mesh->vertex_indices.count)
                            {
                                ...
                                if
                                (
                                    fbx_mesh->vertex_normal.exists &&
                                    ix < fbx_mesh->vertex_normal.indices.count
                                )
                                {
                                ...
                                if
                                (
                                    fbx_mesh->vertex_uv.exists &&
                                    ix < fbx_mesh->vertex_uv.indices.count
                                )
                                {
                ...
                RayMeshes.emplace_back(NewMesh);
            }
        }
        ...
        Model NewModel = {};
        NewModel.meshCount = static_cast<int>(RayMeshes.size());
        NewModel.meshes = static_cast<Mesh*>(MemAlloc(NewModel.meshCount * sizeof(Mesh)));

        // Materials
        NewModel.materialCount = 1;
        NewModel.materials = static_cast<Material*>(MemAlloc(NewModel.materialCount * sizeof(Material)));
        ...
        NewModel.meshMaterial = static_cast<int*>(MemAlloc(NewModel.meshCount * sizeof(int)));
        for (int i = 0; i < NewModel.meshCount; i++)
